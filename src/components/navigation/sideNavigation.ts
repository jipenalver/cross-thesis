// 👉 Main Navigation; Title, Icon
export const mainNav = [
  ['User Management', 'mdi-account-box-multiple'],
  ['Students Management', 'mdi-account-school'],
]

// 👉 Sub Navigations; Title, Icon, Subtitle, Redirect Path
export const menuItemsNav1 = [
  ['User Roles', 'mdi-tag-multiple', 'Add and Manage Roles', '/manage/user/roles'],
  ['Users Management', 'mdi-account-multiple', 'Add and Manage Users', '/manage/users'],
]
export const menuItemsNav2 = [
  ['Students Information', 'mdi-book-account', 'View Students', '/students'],
]
